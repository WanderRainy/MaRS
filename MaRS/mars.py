...
from models.clips import clip_contrastive, cross_contrastive, aclip_contrastive
...
class DualEncoderSimMIM(nn.Module):
    ...
    def forward(self, samples, mask_ratio):
        ...
        elif self.contrast_type == 'aclip':
            loss_contrast = aclip_contrastive(z_rgb_tokens, z_sar_tokens, t=self.contrast_temp)
        else:
            loss_contrast = 0
...
def build_dual_simmim_swin_attn(model_type='base', contrast_type='clip'):
    if model_type == 'base':
    ...
    encoder_rgb = SwinTransformerForSimMIM(model_name, img_size=512, in_chans=3, pretrained=True)
    encoder_sar = SwinTransformerForSimMIM(model_name, img_size=512, in_chans=1, pretrained=True)
    teacher_encoder = SwinTransformerForSimMIM(model_name, img_size=512, in_chans=3, pretrained=True)
    for p in teacher_encoder.parameters():
        p.requires_grad = False

    return DualEncoderSimMIM(
